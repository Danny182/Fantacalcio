...
function get_data_gioc_by_id($id_gioc){
	$query = "SELECT giocatore.id_giocatore, giocatore.nome, giocatore.cognome, giocatore.ruolo, giocatore.n_maglia, giocatore.data_nascita, giocatore.squadra, giocatore.nazionalita FROM giocatore WHERE giocatore.id_giocatore = '$id_gioc'";
	$ris = mysql_query($query);
	$vet = mysql_fetch_array($ris);
	return $vet;	
}
